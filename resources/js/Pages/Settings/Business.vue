    ...
    <form class="mx-auto max-w-6xl space-y-5 px-4" @submit.prevent="submit">
      <PageHeader :title="t('settings.title')" :description="t('settings.description')">
        <template #actions>
          ...
          <AppButton type="submit" :loading="form.processing" icon="fa-save" data-tour="settings-save">{{ t('actions.save') }}</AppButton>
        </template>
      </PageHeader>

      <section class="ihc-panel p-5" data-tour="settings-business">
        <h2 class="text-lg font-black text-[var(--color-text-primary)]">{{ t('settings.business_data') }}</h2>
        <div class="mt-4 grid gap-4 md:grid-cols-2">
          <label class="block">
            <span class="ihc-label">{{ t('settings.business_name') }}</span>
            <input v-model="form.business_name" class="ihc-field" autocomplete="organization" />
            <InputError :message="form.errors.business_name" />
          </label>
          <label class="block">
            <span class="ihc-label">{{ t('settings.legal_name') }}</span>
            <input v-model="form.legal_name" class="ihc-field" />
            <InputError :message="form.errors.legal_name" />
          </label>
          <label class="block">
            <span class="ihc-label">{{ t('settings.tax_id') }}</span>
            <input v-model="form.tax_id" inputmode="numeric" maxlength="11" class="ihc-field" />
            <InputError :message="form.errors.tax_id" />
          </label>
          <label class="block">
            <span class="ihc-label">{{ t('settings.phone') }}</span>
            <input v-model="form.phone" class="ihc-field" autocomplete="tel" />
            <InputError :message="form.errors.phone" />
          </label>
          <label class="block md:col-span-2">
            <span class="ihc-label">{{ t('settings.address') }}</span>
            <input v-model="form.address" class="ihc-field" autocomplete="street-address" />
            <InputError :message="form.errors.address" />
          </label>
          <label class="block">
            <span class="ihc-label">{{ t('settings.email') }}</span>
            <input v-model="form.email" type="email" class="ihc-field" autocomplete="email" />
            <InputError :message="form.errors.email" />
          </label>
          <div>
            <span class="ihc-label">{{ t('settings.logo') }}</span>
            <div class="mt-1 flex flex-wrap items-center gap-3">
              <div class="flex h-16 w-28 items-center justify-center rounded-md border border-[var(--color-border)] bg-[var(--color-surface-alt)]">
                <img v-if="hasLogo" :src="logoPreview" :alt="t('settings.logo')" class="max-h-14 max-w-24 object-contain" />
                <strong v-else class="text-sm text-[var(--color-text-primary)]">{{ form.business_name || 'LaraTory' }}</strong>
              </div>
              <label class="inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm font-bold text-[var(--color-text-primary)] transition hover:-translate-y-0.5 hover:border-[var(--color-primary)] hover:bg-[var(--color-surface-alt)]">
            ...
            <p class="app-ui-help">{{ t('settings.logo_help') }}</p>
            <InputError :message="form.errors.logo_path" />
          </div>
        </div>
      </section>

      <section class="ihc-panel p-5" data-tour="settings-format">
        <h2 class="text-lg font-black text-[var(--color-text-primary)]">{{ t('settings.regional_format') }}</h2>
        <div class="mt-4 grid gap-4 md:grid-cols-4">
          <label>
            <span class="ihc-label">{{ t('settings.currency') }}</span>
            <input v-model="form.currency_code" maxlength="3" class="ihc-field uppercase" />
            <InputError :message="form.errors.currency_code" />
          </label>
          <label>
            <span class="ihc-label">{{ t('settings.symbol') }}</span>
            <input v-model="form.currency_symbol" class="ihc-field" />
            <InputError :message="form.errors.currency_symbol" />
          </label>
          <label>
            <span class="ihc-label">{{ t('settings.timezone') }}</span>
            <input v-model="form.timezone" class="ihc-field" />
            <InputError :message="form.errors.timezone" />
          </label>
          <label>
            <span class="ihc-label">{{ t('settings.decimals') }}</span>
            <input v-model.number="form.decimal_point" type="number" min="0" max="8" class="ihc-field" />
            <InputError :message="form.errors.decimal_point" />
          </label>
          <label>
            <span class="ihc-label">{{ t('settings.date_format') }}</span>
            <select v-model="form.date_format" class="ihc-field">
          ...
          <label>
            ...
            <select v-model="form.time_format" class="ihc-field">
          ...
          <label>
            ...
            <input v-model.number="form.tax" type="number" min="0" max="100" step="0.01" class="ihc-field" />
            <InputError :message="form.errors.tax" />
          </label>
          <label>
            <span class="ihc-label">{{ t('settings.discount') }}</span>
            <input v-model.number="form.discount" type="number" min="0" step="0.01" class="ihc-field" />
            <InputError :message="form.errors.discount" />
          </label>
        </div>
      </section>

      <section class="ihc-panel p-5" data-tour="settings-printing">
        <h2 class="text-lg font-black text-[var(--color-text-primary)]">{{ t('settings.documents_printing') }}</h2>
        <div class="mt-4 grid gap-4 md:grid-cols-3">
          <label class="md:col-span-3">
            <span class="ihc-label">{{ t('settings.footer_message') }}</span>
            <input v-model="form.receipt_footer" class="ihc-field" />
            <InputError :message="form.errors.receipt_footer" />
          </label>
          <label>
            <span class="ihc-label">{{ t('settings.default_document') }}</span>
            <select v-model="form.default_sale_document" class="ihc-field">
          ...
          <label>
            ...
            <select v-model.number="form.thermal_paper_width" class="ihc-field">
          ...
          <label>
            ...
            <input v-model.number="form.print_copies" type="number" min="1" max="5" class="ihc-field" />
        ...
        <div class="mt-5 grid gap-3 md:grid-cols-2">
          ...
          ]" :key="field[0]" class="flex min-h-12 items-center justify-between gap-4 rounded-md border border-[var(--color-border)] bg-[var(--color-surface-alt)] px-4 py-3">
            <span class="font-bold text-[var(--color-text-primary)]">{{ field[1] }}</span>
            <input v-model="form[field[0]]" type="checkbox" class="rounded border-[var(--color-border)] text-[var(--color-primary)] focus:ring-[var(--color-primary)]" />
          </label>
        </div>
      </section>

      <section class="ihc-panel p-5">
        <h2 class="text-lg font-black text-[var(--color-text-primary)]">{{ t('cash.title') }}</h2>
        <label class="mt-4 block max-w-md">
          <span class="ihc-label">{{ t('settings.register_name') }}</span>
          <input v-model="form.cash_register_name" class="ihc-field" />
