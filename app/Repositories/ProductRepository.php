    ...
    private function getQuery(array $filters): Builder|HigherOrderWhenProxy
    {
        return Product::query()
            ->when(isset($filters[ProductFiltersEnum::KEYWORD->value]), function ($query) use ($filters) {
                $keyword = trim((string) $filters[ProductFiltersEnum::KEYWORD->value]);

                $query->where(function (Builder $keywordQuery) use ($keyword) {
                    if (is_numeric($keyword)) {
                        $keywordQuery->where(ProductFieldsEnum::ID->value, $keyword);
                    }

                    $keywordQuery
                        ->orWhere(ProductFieldsEnum::NAME->value, 'like', '%'.$keyword.'%')
                        ->orWhere(ProductFieldsEnum::DESCRIPTION->value, 'like', '%'.$keyword.'%')
                        ->orWhere(ProductFieldsEnum::PRODUCT_NUMBER->value, $keyword)
                        ->orWhere(ProductFieldsEnum::PRODUCT_CODE->value, $keyword)
                        ->orWhere(ProductFieldsEnum::PRODUCT_CODE->value, 'like', '%'.$keyword.'%')
                        ->orWhere(ProductFieldsEnum::BARCODE->value, $keyword)
                        ->orWhere(ProductFieldsEnum::BARCODE->value, 'like', '%'.$keyword.'%');
                });
            })
            ...
            ->when(isset($filters[ProductFiltersEnum::BARCODE->value]), function ($query) use ($filters) {
                $query->where(ProductFieldsEnum::BARCODE->value, $filters[ProductFiltersEnum::BARCODE->value]);
            })
